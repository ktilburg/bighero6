  ...
  if (isOpening) {
    ...
    navLinks.animate(
      [
        ...
        {
          height: navLinks.scrollHeight + "px",
          opacity: 1,
...
function movePersona(direction) {
  const track = document.getElementById("personaTrack");
  const groups = track.getElementsByClassName("persona-group");

  personaIndex += direction;

  if (personaIndex < 0) {
    personaIndex = groups.length - 1;
  } else if (personaIndex >= groups.length) {
    personaIndex = 0;
  }

  const amount = -personaIndex * 60;
